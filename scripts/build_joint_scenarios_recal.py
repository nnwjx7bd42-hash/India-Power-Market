...
def run_build_scenarios():
    ...
    dam_df.to_parquet(pred_dir / "joint_dam_scenarios_backtest_recalibrated.parquet", index=False)
    dam_df.to_csv(pred_dir / "joint_dam_scenarios_backtest.csv", index=False)
    rtm_df.to_parquet(pred_dir / "joint_rtm_scenarios_backtest_recalibrated.parquet", index=False)
    rtm_df.to_csv(pred_dir / "joint_rtm_scenarios_backtest.csv", index=False)
    
    # Also export existing quantiles to CSV
    for f in ["dam_quantiles_backtest_recalibrated.parquet", "rtm_quantiles_backtest_recalibrated.parquet"]:
        path = pred_dir / f
        if path.exists():
